...
<template>
  ...
  >
    <svg viewBox="0 0 24 24" fill="none">
      ...
      <circle class="themeToggleBody" cx="12" cy="12"
              fill="currentColor" mask="url(#morph-mask)"/>
      <g class="themeToggleRays"
...
.themeToggleBody {
  r: 5;
  transition: r .5s cubic-bezier(.5,1.4,.4,1);
...
.themeToggleCut {
  cx: 40;
  cy: -6;
  transition: cx .55s cubic-bezier(.5,1.4,.4,1), cy .55s cubic-bezier(.5,1.4,.4,1);

  .themeToggle.on & { cx: 18; cy: 8; }
}
...
.themeToggleRays {
  ...
  opacity: 1;
  transition: transform .45s cubic-bezier(.5,1.5,.4,1), opacity .25s ease;
